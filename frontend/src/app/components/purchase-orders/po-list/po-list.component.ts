...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, MatTableModule, MatButtonModule, MatIconModule, MatSelectModule, MatFormFieldModule, MatPaginatorModule, MatSnackBarModule, MatProgressSpinnerModule, MatTooltipModule],
    template: `
    <div class="page-container fade-in">
      <div class="page-header">
        <div><h1 class="page-title">Purchase Orders</h1><p class="page-subtitle">Manage vendor purchase orders</p></div>
        <div style="display:flex;gap:8px" *ngIf="authService.hasRole('ADMIN','MANAGER')">
      ...
      <div *ngIf="!loading" class="table-container" style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden">
        <table mat-table [dataSource]="orders" style="width:100%">
          ...
          <ng-container matColumnDef="actions"><th mat-header-cell *matHeaderCellDef>Actions</th>
            <td mat-cell *matCellDef="let o"><div style="display:flex;gap:4px" *ngIf="authService.hasRole('ADMIN','MANAGER')">
              ...
              <button mat-icon-button matTooltip="Dispatch" (click)="dispatch(o)" *ngIf="o.status==='APPROVED'"><mat-icon>local_shipping</mat-icon></button>
...
export class PoListComponent implements OnInit {
    orders: any[] = []; cols = ['id', 'product', 'vendor', 'quantity', 'status', 'created_at', 'actions'];
    ...
    approve(o: any) { this.api.approvePO(o.id).subscribe({ next: () => { this.snackBar.open('Approved', 'Close', { duration: 3000 }); this.loadPOs() }, error: e => this.snackBar.open(e.error?.error || 'Failed', 'Close', { duration: 3000 }) }); }
    dispatch(o: any) { this.api.dispatchPO(o.id).subscribe({ next: () => { this.snackBar.open('Dispatched', 'Close', { duration: 3000 }); this.loadPOs() }, error: e => this.snackBar.open(e.error?.error || 'Failed', 'Close', { duration: 3000 }) }); }
    reject(o: any) { if (confirm('Reject?')) this.api.rejectPO(o.id).subscribe({ next: () => { this.snackBar.open('Rejected', 'Close', { duration: 3000 }); this.loadPOs() }, error: e => this.snackBar.open(e.error?.error || 'Failed', 'Close', { duration: 3000 }) }); }
    checkRestock() { this.api.checkRestock().subscribe({ next: r => { this.snackBar.open(`${r.generated} POs generated`, 'Close', { duration: 5000 }); this.loadPOs() }, error: e => this.snackBar.open(e.error?.error || 'Failed', 'Close', { duration: 3000 }) }); }
}
